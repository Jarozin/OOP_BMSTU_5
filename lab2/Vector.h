...
public:
    ...
    template<typename S> Vector<Type>& operator =(const Vector<S>& vec);
    ...
    template<typename S> Vector<Type>& operator *=(const S& mult);
    template<typename S> decltype(auto) operator *(const S& mult) const;
    template<typename S> Vector<Type>& mult_num(const S& mult);

    template<typename S> Vector<Type>& operator /=(const S& div);
    template<typename S> decltype(auto) operator /(const S& div) const;
    template<typename S> Vector<Type>& div_num(const S& mult);

    //унарный минус
    Vector<Type> operator -();

    //скалярное умнжение
    template<class S> decltype(auto) operator ^(const Vector<S>& vec) const;
    template<typename S> decltype(auto) mult_vect_scalar(const Vector<S> &vec2) const;

    //векторное умножение
    template<typename S> Vector<Type>& operator &=(const Vector<S>& vec);
    template<typename S> decltype(auto) operator &(const Vector<S>&) const;
    template<typename S> Vector<Type>& mult_vect_cross(const Vector<S> &vec2);


    //поэлементное сложение
    template<typename S> decltype(auto) operator +(const Vector<S>&) const;
    template<typename S> Vector<Type>& operator +=(const Vector<S>&);
    template<typename S> Vector<Type>& add(const Vector<S>&);
    template<typename S, typename R> static decltype(auto) sum_vectors(const Vector<R>& vec1, const Vector<S>& vec2);

    //поэлементное вычитание
    template<typename S> decltype(auto) operator -(const Vector<S>&) const;
    template<typename S> Vector<Type>& sub(const Vector<S>&);
    template<typename S> Vector<Type>& operator -=(const Vector<S>&);
    template <typename S, typename R> static decltype(auto) difference_vectors(const Vector<R>& vec1, const Vector<S>& vec2);
